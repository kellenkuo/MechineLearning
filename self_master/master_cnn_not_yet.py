...
class Model( object ):
    ...
    def __init__( self, kernel_size1 = [ 5, 5 ], num_of_filter = 16 ):
        # define variable 
    ...
    def Convolution_init( self, kernel_size1, num_of_filter ):
        ...
        # create init weights matrix
        tmp_matrix = np.zeros_like( self.Convolution_layer1['weights'] )
        for i in range( self.Nodes['Convolution_layer1'] ):
            for k in range(784):
                if k in final[i]:
                    tmp_matrix[k][i] = 1
        self.Convolution_layer1['final'] = tmp_matrix
        # set weights to zero
        self.Convolution_layer1['weights'] *= self.Convolution_layer1['final']
    ...
    def GradientDescentOptimizer( self, LearningRate = 0.01 ):
        ...
        self.layer2['delta'] = layer2_Error * self.sigmoid( self.layer2['prediction'], derivative = True )
        layer1_Error = self.layer2['delta'].dot( self.layer2['weights'].T )
        self.layer1['delta'] = layer1_Error * self.relu( self.layer1['prediction'], derivative = True )
        Convolution_layer1_Error = self.layer1['delta'].dot( self.layer1['weights'].T )
        self.Convolution_layer1['delta'] = Convolution_layer1_Error * self.relu( self.Convolution_layer1['output'], derivative = True )
    ...
    def train( self, dataset, num_of_training = 40000, print_process = True ):
        ...
        for loop in range( num_of_training ):
            ...
            # forward
            self.Convolution_layer1['output'] = self.relu( np.dot( self.input_layer, self.Convolution_layer1['weights'] ) )
            self.layer1['prediction'] = self.relu( np.dot( self.Convolution_layer1['output'], self.layer1['weights'] ) )
            self.layer2['prediction'] = self.sigmoid( np.dot( self.layer1['prediction'], self.layer2['weights'] ) )
    ...
    def test( self, dataset, num_of_testing = 1000 ):
        ...
        for loop in range( num_of_testing ):
            ...
            if np.argmax( self.layer2['prediction'] ) == np.argmax( self.labels ):
                correct += 1
        return round( correct / num_of_testing, 6 )
    ...
    def PRINT_RATE( self, i, loop ):
        ...
        print( "\033[3;36H{}".format( round( ( loop / i )*100 , 2 ) ) )
...
if __name__ == '__main__':
    ...
    for i in range( epochs ):
        ...
        #Network.train( mnist.train, num_of_training = 40000, print_process = True )
